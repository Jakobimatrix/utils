...
BOOST_AUTO_TEST_CASE(create_variants_test) {
  ...
  using MyVariant_0 = TypeContainerCreater<std::variant, false, int, double, float>::
      SingleTypeStl<std::set, std::vector>::PairedTypeStl<std::pair>::type;
  using MyTuples_0 =
      TypeContainerCreater<std::tuple, true, int, double, float>::SingleTypeStl<std::set, std::vector>::PairedTypeStl<std::pair>::type;
